  ...
  useEffect(() => {
    async function loadDashboard() {
      try {
        ...
        const [statsData, monthlySalesData, activityData] = await Promise.all([
          getDashboardStats(),
          getMonthlySales(),
          getRecentActivity(),
        ]);
        ...
        <DashboardStatsGrid>
          ...
          <DashboardStatCard
            ...
            value={
              loading
                ? "..."
                : formatCurrency(monthlySales[new Date().getMonth()]?.sales)
            }
          />
        </DashboardStatsGrid>
      </DashboardSection>
                ...
                recentActivity.map((activity) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-gray-400">
                      {activity.type} • {formatActivityDate(activity.date)}
                    </p>
                  </div>
                ))
              )}
